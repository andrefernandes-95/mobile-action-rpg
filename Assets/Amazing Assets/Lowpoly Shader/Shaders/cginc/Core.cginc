...
{
    float3 fWidth = fwidth(mass);
	float3 width = mass - _Lowpoly_WireframeThickness.xxx;

	float3 df1 = smoothstep(0, fWidth, width);	
	float w1 = min(min(df1.x, df1.y), df1.z);

	float3 df2 = width / (fWidth * (1 + _Lowpoly_WireframeSmoothness * 3) + 1e-6);
	float w2 = smoothstep(0.0, 1.0, min(min(df2.x, df2.y), df2.z) + 0.5);
	w2 *= w2;
    

	return 1 - lerp(w1, w2, _Lowpoly_WireframeSmoothness);
}
#endif
